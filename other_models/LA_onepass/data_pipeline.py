...
def save_preds(truthfile, truths,seed_number,pass_type='one'):
    ground_truths = {}
    f = open(truthfile,'r')
    reader = csv.reader(f)
    next(reader)
    ...
    assert truths.keys() == ground_truths.keys(), "Dictionaries must have the same keys"
    assert len(truths) == len(ground_truths), "Dictionaries must have the same number of elements"

    sorted_keys = sorted(truths.keys(), key=int)  # Sort numerically if keys are numbers as strings

    import numpy as np
    la_preds = np.array([int(truths[k]) for k in sorted_keys])
    true_labels = np.array([int(ground_truths[k]) for k in sorted_keys])
    np.savez(f'la_{pass_type}_preds_s{seed_number}.npz', la_preds=la_preds, true_labels=true_labels)
